...
export function PlayerSettingsPanel() {
  ...
  async function testSend() {
    setTesting(true)
    try {
      const r = await fetch('/api/admin/messages/send', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ kind: 'test', message: 'رسالة تجربة من منصة Maths Genius — لو وصلتك يبقى الإرسال شغال ✅' }) })
      const d = await r.json()
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <Label className="text-sm">ارتفاع الدرع العلوي (شريط Math Genius فوق)</Label>
