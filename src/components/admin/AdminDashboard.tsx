      ...
      <div className="mx-auto max-w-7xl">
        ...
        <Tabs value={adminTab} onValueChange={setAdminTab} className="space-y-6">
          <TabsList className="flex flex-wrap h-auto gap-1 bg-muted/50 p-1">
            ...
            <TabsTrigger value="hw-results" className="text-xs sm:text-sm gap-1 text-emerald-600 dark:text-emerald-400"><CheckCircle2 className="h-4 w-4" /><span className="hidden sm:inline">نتائج الواجبات</span></TabsTrigger>
          ...
          <TabsContent value="hw-results"><HomeworkResultsPanel /></TabsContent>
...
/* ========== HOMEWORK RESULTS PANEL (results tracking for admin) ========== */
function HomeworkResultsPanel() {
  ...
  const loadHomeworks = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/homework?pageSize=100')
      const data = await res.json()
  ...
  useEffect(() => { loadHomeworks() }, [])

  const loadResults = async (hwId: string) => {
    if (!hwId) { setResults([]); setNotSubmitted([]); setHwInfo(null); setAvgScore('—'); return }
    try {
      const res = await fetch(`/api/homework-results?homeworkId=${hwId}`)
      const data = await res.json()
      setResults(data.results || [])
      setNotSubmitted(data.notSubmitted || [])
      setHwInfo(data.hwInfo || null)
      setAvgScore(data.avgScore || '—')
    } catch { toast.error('خطأ في تحميل النتائج') }
  }

  const handleHwSelect = (hwId: string) => {
    setSelectedHw(hwId)
    loadResults(hwId)
  }
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-lg flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-emerald-500" />تتبع نتائج الواجبات</CardTitle>
      </CardHeader>
      <CardContent>
        {loading ? <div className="flex justify-center py-10"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : (
          <div className="grid gap-4 lg:grid-cols-3">
            {/* Homework List */}
            <div className="space-y-2 max-h-[600px] overflow-y-auto custom-scrollbar lg:col-span-1">
              <p className="text-xs font-medium text-muted-foreground mb-2">اختر واجب لعرض النتائج</p>
              {homeworks.length === 0 ? <p className="text-xs text-muted-foreground text-center py-6">لا توجد واجبات</p> : homeworks.map((hw) => (
                <div key={hw.id} className={`p-3 rounded-lg border cursor-pointer transition-colors hover:bg-muted/50 ${selectedHw === hw.id ? 'border-emerald-500 bg-emerald-50/50 dark:bg-emerald-900/10' : 'bg-card'}`}
                  onClick={() => handleHwSelect(hw.id)}>
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <p className="font-medium text-sm truncate">{hw.title}</p>
                      <p className="text-[10px] text-muted-foreground">{hw.grade}</p>
                      <div className="flex gap-1 mt-1">
                        {(hw as any).filePath && <Badge variant="outline" className="text-[9px] border-primary/40 text-primary">ملف</Badge>}
                        {(hw as any).questions && (hw as any).questions !== '' && <Badge variant="outline" className="text-[9px] border-emerald-500/40 text-emerald-600">أسئلة</Badge>}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="lg:col-span-2 space-y-4">
              {!selectedHw ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">اختر واجب من القائمة لعرض النتائج</p>
                </div>
              ) : (
                <>
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="text-center px-4 py-2 rounded-lg bg-primary/10"><p className="text-lg font-bold text-primary">{results.length}</p><p className="text-[10px] text-muted-foreground">سلموا</p></div>
                    <div className="text-center px-4 py-2 rounded-lg bg-emerald-500/10"><p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{avgScore}</p><p className="text-[10px] text-muted-foreground">متوسط الدرجات</p></div>
                    <div className="text-center px-4 py-2 rounded-lg bg-red-500/10"><p className="text-lg font-bold text-red-600 dark:text-red-400">{notSubmitted.length}</p><p className="text-[10px] text-muted-foreground">لم يسلموا بعد</p></div>
                  </div>
                  {hwInfo && (
                    <div className="flex items-center gap-2 flex-wrap text-[10px] text-muted-foreground bg-muted/30 p-2 rounded-lg">
                      <Badge variant="outline" className="text-[9px] border-blue-500/40 text-blue-600">اختياري: {hwInfo.totalMcq || 0}</Badge>
                      {hwInfo.totalWriting > 0 && <Badge variant="outline" className="text-[9px] border-amber-500/40 text-amber-600">مقالي: {hwInfo.totalWriting}</Badge>}
                      <span>الصف: {hwInfo.grade || '—'}</span>
                    </div>
                  )}
                  {results.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-medium text-muted-foreground">الطلاب الذين سلموا الواجب (اضغط على الطالب لرؤية التفاصيل)</p>
                      <div className="max-h-[500px] overflow-y-auto custom-scrollbar space-y-2">
                        {results.map((r: any) => {
                          var isExpanded = expandedResult === r.id
                          var allQs = r.allQuestions || []
                          var writingAns = r.writingAnswers || []
                          var wrongQs = r.wrongQuestions || []
                          var passed = r.score >= (r.maxScore / 2)
                          return (
                            <div key={r.id} className={`rounded-lg border overflow-hidden transition-all ${isExpanded ? 'border-emerald-500' : 'border-border bg-card'}`}>
                              <button
                                type="button"
                                onClick={() => setExpandedResult(isExpanded ? null : r.id)}
                                className="w-full flex items-center justify-between p-3 text-right hover:bg-muted/30 transition-colors"
                              >
                                <div className="flex items-center gap-2 min-w-0 flex-1">
                                  <div className={"h-8 w-8 rounded-lg flex items-center justify-center shrink-0 " + (passed ? 'bg-emerald-500/10' : 'bg-red-500/10')}>
                                    <span className={"text-xs font-bold " + (passed ? 'text-emerald-600' : 'text-red-500')}>{r.score}/{r.maxScore}</span>
                                  </div>
                                  <div className="min-w-0 text-right">
                                    <p className="font-medium text-sm truncate">{r.student?.name || '—'}</p>
                                    <div className="flex items-center gap-2 mt-0.5">
                                      <span className="text-[10px] text-muted-foreground" dir="ltr">{r.student?.phone || ''}</span>
                                      <span className="text-[10px] text-muted-foreground">{r.submittedAt ? new Date(r.submittedAt).toLocaleDateString('ar-EG') : ''}</span>
                                      {wrongQs.length > 0 && (
                                        <Badge variant="outline" className="text-[9px] border-red-500/40 text-red-600">{wrongQs.length} خطأ</Badge>
                                      )}
                                      {writingAns.length > 0 && (
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  <Badge variant={passed ? 'default' : 'outline'} className={`text-[9px] h-5 ${passed ? 'bg-emerald-500' : 'border-amber-500 text-amber-600'}`}>
                                    {passed ? 'ناجح' : 'محتاج مراجعة'}
                                  </Badge>
                                  <ChevronLeft className={"h-4 w-4 text-muted-foreground transition-transform " + (isExpanded ? 'rotate-90' : '')} />
                                </div>
                              </button>
                  ...
                  {notSubmitted.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-medium text-red-500">لم يسلموا الواجب بعد</p>
                      <div className="max-h-[200px] overflow-y-auto custom-scrollbar space-y-1">
                        {notSubmitted.map((s) => (
                          <div key={s.id} className="flex items-center justify-between p-2 rounded-lg border border-red-200 dark:border-red-900/30 bg-red-50/50 dark:bg-red-900/10 text-sm">
                            <div><span className="font-medium">{s.name}</span> <span className="text-[10px] text-muted-foreground" dir="ltr">{s.phone}</span></div>
