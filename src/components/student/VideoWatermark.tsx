...
export function VideoWatermark({ name, phone }: { name?: string; phone?: string }) {
  ...
  return (
    ...
    >
      ...
      {/* كارت ثابت فوق مكان لوجو يوتيوب (الركن الشفق السفلي للفيديو) —
          طلب المستر: أي علامة يوتيوب تتغطى بكارت اسم الطالب. دايمًا ظاهر. */}
      <div
        className="absolute"
        style={{ right: '2%', bottom: '6.5%', transform: 'rotate(-10deg)' }}
      >
        <div
          className="wm-bob2 rounded-xl text-center"
          style={{
            direction: 'rtl',
        ...
        >
          <p className="text-[11px] sm:text-xs font-black text-white leading-tight" style={{ direction: 'ltr', unicodeBidi: 'plaintext' }}>{num}</p>
          <p className="text-[9px] sm:text-[10px] font-bold text-white/90 leading-tight">{nm}</p>
        </div>
      </div>
