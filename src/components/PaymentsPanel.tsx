...
import {
  Wallet, Check, X, Loader2, Eye, Trash2, Smartphone, CreditCard, Wallet as FawryIcon,
  Image as ImageIcon, Search, Filter, Clock, CheckCircle2, XCircle, Receipt
} from 'lucide-react'
...
const methodLabels: Record<string, { label: string; icon: any; color: string }> = {
  ...
  fawry: { label: 'فوري', icon: FawryIcon, color: 'text-blue-600 bg-blue-100 dark:bg-blue-900/30' },
}
...
export function PaymentsPanel({ onRefresh }: { onRefresh: () => void }) {
  ...
  var handleViewReceipt = async function(paymentId: string) {
    ...
    try {
      // receiptPath stores the media ID
      var res = await fetch('/api/files/' + payment.receiptPath)
      if (res.ok) {
        var blob = await res.blob()
        setReceiptMediaId(URL.createObjectURL(blob))
      } else {
  ...
  return (
    <>
          ...
          {loading ? (
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto custom-scrollbar">
              {filteredPayments.map(function(p) {
                var method = methodLabels[p.paymentMethod] || { label: p.paymentMethod, icon: Wallet, color: 'text-muted-foreground bg-muted' }
                  ...
                  <div key={p.id} className={"p-4 rounded-xl border transition-all " + (p.status === 'pending' ? 'border-amber-200 dark:border-amber-800/50 bg-amber-50/50 dark:bg-amber-900/10' : 'bg-card') }>
                    ...
                    {p.notes && (
                      <div className="mb-3 p-2 rounded-lg bg-muted/50 text-xs text-muted-foreground">
                        <span className="font-medium">ملاحظات الطالب:</span> {p.notes}
                      </div>
                    )}
                      ...
                      {p.status === 'pending' && (
                        <>
                          <Button
                            ...
                            onClick={function() { setShowApproveConfirm(p.id) }}
                          ...
                          <Button
                            ...
                            onClick={function() { setShowRejectNotes(p.id); setRejectNotes('') }}
                            disabled={actionLoading === p.id}
                          >
      ...
      {viewingReceipt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={function() { setViewingReceipt(null); if (receiptMediaId) URL.revokeObjectURL(receiptMediaId); setReceiptMediaId(null) }}>
          <div className="bg-card border rounded-2xl p-4 w-full max-w-lg mx-4 shadow-2xl" onClick={function(e) { e.stopPropagation() }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={function() { setViewingReceipt(null); if (receiptMediaId) URL.revokeObjectURL(receiptMediaId); setReceiptMediaId(null) }}>
                <X className="h-4 w-4" />
              </Button>
            </div>
            {receiptLoading ? (
              ...
              <div className="relative rounded-lg overflow-hidden border bg-black/5 aspect-[3/4] max-h-[70vh]">
                <Image src={receiptMediaId} alt="إيصال الدفع" fill className="object-contain" unoptimized />
              </div>
            ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={function() { setShowRejectNotes(null) }}>
          <div className="bg-card border rounded-2xl p-6 w-full max-w-sm mx-4 shadow-2xl" onClick={function(e) { e.stopPropagation() }}>
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <Textarea
                value={rejectNotes}
                onChange={function(e) { setRejectNotes(e.target.value) }}
                placeholder="مثال: الإيصال غير واضح، ارفع إيصال آخر..."
                rows={3}
