...
export async function POST(request: NextRequest) {
  try {
    ...
    // المساحات»: ملفات الطلاب المرفوعة بتراكم سريعًا (كانت 474MB في 11 يوم!)
    // فبتعيش أسبوع للمذاكرة/التقييم وبعدها بتنضف تلقائيًا من auto-clean،
    // والقاعدة دي للمخزون القديم من الأدمن. الدرجات (HomeworkResult) مش بتتلمس.
    try {
      ...
      results.push({ table: 'Media (حلول واجبات أقدم من أسبوع)', found: 0, deleted: 0, err: String((e && e.message) || e).slice(0, 200) })
    }
