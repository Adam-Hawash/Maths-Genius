...
//   • خصوصية: الاسم بيرجع بكلمتين كحد أقصى — ممنوع التليفون أو أي بيانات شخصية
...
export async function GET() {
  ...
  try {
    ...
    // تنسيق الرد: الاسم بكلمتين كحد أقصى + الصف + النقاط كرقم نظيف
    var leaderboard = (rows || []).map(function (r: any) {
      var fullName = String(r.name || 'طالب').trim()
      var words = fullName.split(/\s+/).filter(Boolean)
      var displayName = words.slice(0, 2).join(' ') // خصوصية: كلمتين كحد أقصى
