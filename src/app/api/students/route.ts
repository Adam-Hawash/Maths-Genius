...
function ensureStudentSchema(): Promise<void> {
  if (!schemaReady) {
    schemaReady = (async function () {
      ...
      // ============================================================
      // **إعادة ضبط ربط الأجهزة — لمرة واحدة فقط (علاج حجب كل الأجهزة)**
      // ============================================================
      // المشكلة اللي المستر بلّغ عنها: طلاب (وحتى هو نفسه) بيتحجبوا من
      // **كل** الأجهزة حتى جهازهم الأصلي. السبب: النسخ القديمة من الكود كانت
      // بتعيد كتابة ربط الجهاز في مسارات كتير (آخر جهاز كسب السباق بيبقى
      // "جهاز الحساب") — وترحيل "تثبيت ربط الإنشاء" جمّد البيانات الغلط دي
      // كأنها جهاز إنشاء ثابت → الحساب بقى مقفول على جهاز مش جهاز صاحبه
      // نهائيًا، ومفيش أي طريق إنقاذ بيوصله (الهوية غلط + البصمة غلط +
      // مكوّنات الجهاز المخزنة غلط كلهم لنفس الجهاز الغريب).
      //
      // الحل: مسح كل الروابط القديمة **مرة واحدة** (علم في SiteConfig بيمنع
      // تكرارها)، وبكده أول جهاز يدخل بكل حساب بعد التحديث بيبقى جهازه
      // الرسمي وبيتقفل عليه — بالظبط زي معنى لافتة الدخول اللي المستر وافق
      // عليها: "الجهاز اللي هتدخل منه دلوقتي هو ده جهازك على طول".
      // الطلاب المحجوبين بيرجعوا يدخلوا عادي، والقاعدة (جهاز واحد للحساب)
      // بتفضل شغالة زي ما هي بعد أول دخول.
      try {
        var flag = null as any
        try { flag = await db.siteConfig.findUnique({ where: { key: 'device_reset_v3' } }) } catch (e0) {}
        if (!flag || flag.value !== '1') {
          try {
            await db.student.updateMany({
              data: { deviceId: '', deviceFp: '', creationDeviceId: '', creationDeviceFp: '', deviceTraits: '' },
            })
            try {
              await db.siteConfig.create({ data: { key: 'device_reset_v3', value: '1' } })
            } catch (e1) {
              try { await db.siteConfig.update({ where: { key: 'device_reset_v3' }, data: { value: '1' } }) } catch (e2) {}
            }
            console.log('[device-reset-v3] تم مسح كل روابط الأجهزة القديمة لمرة واحدة — أول دخول بعد كده بيربط الحساب بجهازه')
          } catch (e3) {
            // لو المسح فشل منضمّن منعش العلم — المحاولة هتتكرر في الطلب الجاي
            console.error('[device-reset-v3] failed:', e3)
          }
        }
      } catch (eReset) {}
    })()
