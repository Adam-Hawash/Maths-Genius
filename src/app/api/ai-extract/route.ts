...
function buildSingleFilePrompt(grade: string, type: string): string {
  ...
  lines.push('  * Fractions: write every fraction in STACKED form across three lines inside the text:')
  lines.push('    line 1 = numerator, line 2 = a separator row of ─ characters (same length as the longer of the two numbers), line 3 = denominator.')
  lines.push('    In the JSON string use \\n for the line breaks.')
  lines.push('    Example — three quarters must appear EXACTLY as: "3\\n─\\n4"')
...
function buildQuestionsOnlyPrompt(grade: string, type: string): string {
  ...
  lines.push('- Fractions: write every fraction in STACKED form across three lines inside the question text:')
  lines.push('  numerator on its own line, then a separator row of \u2500 characters (same length as the longer of the two numbers), then denominator on its own line. Use \\n in the JSON string for the line breaks.')
  lines.push('  Example \u2014 three quarters must appear EXACTLY as: "3\\n\u2500\\n4"')
...
function buildAnswersOnlyPrompt(grade: string, type: string, questions: any[]): string {
  ...
  lines.push('- Fractions: in modelAnswer / acceptedAnswers write fractions in STACKED form across three lines (numerator, then a ─ separator row, then denominator) using \\n line breaks in the JSON string. Example: "3\\n─\\n4".')
