...
export async function POST(request: Request) {
  try {
    ...
    // **النصوص: Gemini الأول (استريم حقيقي — أول حرف بيوصله وهو نازل من الموديل)
    // → ZAI احتياطي (مفيش مفاتيح ورده قوية)
    // **الصور: Gemini الأول (vision مثبت) → ZAI احتياطي
    // (و77) sseForward: في وضع SSE للنصوص بس، بتتحط جوه الستريم عشان كل قطعة
    // من الموديل تتبعت للطالب فورًا — من غير انتظار الرد الكامل ولا تايب رايتر */
    var sseForward: ((t: string) => void) | null = null
      ...
      if (hasGeminiKey()) {
        engines.push(function () {
          return streamGemini({
            ...
            timeoutMs: timeoutMs,
            thinking: 'low',
            onDelta: function (t: string) { if (sseForward) sseForward(t) },
          })
        })
      }
    ...
    if (useStream) {
      ...
      var acc = '' // (و77) قطع الاستريم الحقيقي اللي وصلت فعلًا للطالب
      var stream = new ReadableStream({
        async start(controller) {
          ...
          if (result) {
            ...
            if (acc) {
              /* (و77) الاستريم الحقيقي وصل للطالب قطعة قطعة وهو نازل من الموديل —
                 ممنوع إعادة إرسال النص بتايب رايتر (هيتكرر مرتين) — نقفل بس */
              send({ done: true })
            } else {
