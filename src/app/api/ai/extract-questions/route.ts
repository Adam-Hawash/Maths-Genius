...
export async function POST(request: NextRequest) {
  try {
    ...
    var prompt = 'سؤال اكتب: رقم السؤال، نص السؤال كاملاً، الإجابة الصحيحة، و4 اختيارات (إذا كان EVERY أنت معلم رياضيات خبير. استخرج كل الأسئلة من هذه الصفحة/الصورة. الملف. لـ "نص السؤال": "question", "options": ["اختيار1", "اختيار2", "اختيار3", "اختيار4"], "correct": 0}; فقط بدون أي نص إضافي بهذا الشكل JSON كان سؤال اختيار من متعدد) أو "لا يوجد" إذا كان سؤال حر. رد بـ كلها "لا يوجد الاختيار الصحيح (0-3)، إذا السؤال حر حط index = correct : ملاحظات . correct [0]{اختيارة 1, "اختيار2", "اختيار3", "اختيار4"] options. لو فيها معادلات اكتبها بالعادي، استخرج كل الأسئلة الموجودة، 0 = correct'
    ...
    if (isPdf) {
      parts.push({ inlinedData: { mimeType: 'application/pdf', data: imageBase64 } })
    } else {
      if (!mimeType || !mimeType.startsWith('image/')) {
        mimeType = 'image/png'
      }
      parts.push({ inlinedData: { mimeType: mimeType, data: imageBase64 } })
    }
    ...
    if (!response.ok) {
      ...
      if (isPdf) {
        ...
        return NextResponse.json({ error: 'فشل استخراج الأسئلة من ملف .PDF. جرب ترفع صورة للأأسئلة بدل .PDF' }, { status: 500 })
      }
