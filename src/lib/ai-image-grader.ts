...
const MODELS = ['gemini-3.7-flash', 'gemini-3.6-flash', 'gemini-2.0-flash', 'gemini-flash-latest']
...
export async function gradeImageAnswer(params: {
  ...
  var prompt = 'أنت معلم رياضيات خبير ودقيق جداً. الطالب رفع صورة لحله لهذا السؤال:\n\n'
  prompt += 'السؤال: ' + question + '\n\n'
  if (modelAnswer) {
    prompt += 'الإجابة النموذجية الكاملة: ' + modelAnswer + acceptedAnswersStr + '\n\n'
  }
  prompt += 'المطلوب منك بالظبط:\n'
  prompt += '1. اقرأ الصورة بدقة شديدة جداً - شوف كل المكتوب فيها (أرقام، رموز، أسس، كسور، معادلات)\n'
  prompt += '2. حدد الإجابة النهائية للطالب - يعني آخر حاجة وصلها الطالب في حله\n'
  prompt += '   - مثلاً لو كتب: (2^3)^4 = 2^12, ثم 2^12 * 2^5 = 2^17, ثم 2^17 / 2^7 = 2^10 = 1024\n'
  prompt += '   - الإجابة النهائية هي: 1024 (أو 2^10)\n'
  prompt += '3. شوف الإجابة النهائية النموذجية (آخر حاجة في الإجابة النموذجية - بعد آخر علامة =)\n'
  prompt += '4. قارن الإجابة النهائية للطالب بالإجابة النهائية النموذجية:\n'
  prompt += '   - لو الإجابة النهائية صح، اعتبر الإجابة صحيحة (isCorrect: true)\n'
  prompt += '   - لو الإجابة النهائية غلط، اعتبر الإجابة خاطئة (isCorrect: false)\n'
  prompt += '5. اقبل الإجابات المكافئة رياضياً:\n'
  prompt += '   - 1024 = 2^10 = 2¹⁰\n'
  prompt += '   - x^4*y^3 = x⁴y³ = x4y3\n'
  prompt += '   - 1/2 = 0.5 = ½\n'
  prompt += '   - √9 = 3\n'
  prompt += '6. استخدم الرموز الرياضية الصحيحة:\n'
  prompt += '   - القسمة: ÷ (مش /)\n'
  prompt += '   - الضرب: × (مش *)\n'
  prompt += '   - الأس: ^ (مثلاً 2^3 يعني 2 مرفوع للقوة 3)\n'
  prompt += '   - باي: π\n'
  prompt += '   - الجذر: √\n'
  prompt += '   - الكسر: استخدم /\n\n'
  prompt += 'ملاحظات مهمة جداً:\n'
  prompt += '- اقرا الصورة كويس جداً - حتى الأرقام الصغيرة والأسس\n'
  prompt += '- ركز على الإجابة النهائية (آخر نتيجة في حل الطالب)\n'
  prompt += '- لو الطالب وصل لنفس النتيجة النهائية بطريقة مختلفة، اعتبرها صحيحة\n'
  prompt += '- لو مكتوب جنب الإجابة النهائية حاجة تانية (مثل "= 1024 والطالب كتب بس 1024")، خد الإجابة النهائية\n'
  prompt += '- لو الطالب كتب بس الإجابة النهائية بدون خطوات، اعتبرها صحيحة لو مطابقة\n\n'
  prompt += 'أرجع النتيجة بصيغة JSON فقط (بدون أي شرح إضافي):\n'
  prompt += '{\n'
  prompt += '  "extractedAnswer": "كل المكتوب في الصورة (نص كامل للحل)",\n'
  prompt += '  "finalAnswer": "الإجابة النهائية للطالب بس (مثلاً: 1024 أو x⁴y³)",\n'
